// Common services not to be changed
var app= angular.module("app.services", [])


app.service('AuthService',['$http', '$rootScope','$cookies','$window','$filter', function($http,$rootScope,$cookies,$window,$filter) {
	
    

	// validate request
	this.validateRequest = function (data,config) {
        return $http.get($rootScope.gatewayRouter+'/iam/authenticate/validateToken',data,config);
	};
	
	// app Switch
	this.appSwitch = function (data,config) {
        return $http.get($rootScope.gatewayRouter+'/iam/authenticate/switchApp?appID='+data.appID,data,config);
	};
	
	
      // Get Org list
	this.getOrgList = function (data,config) {
        return $http.get($rootScope.gatewayRouter+'/iam/organization/getMyOrgs',data,config);
	};
	
	// Change org
	this.changeOrg = function (data,config) {
        return $http.get($rootScope.gatewayRouter+'/iam/organization/switchOrg?orgID='+data.orgID,data,config);
	
	};
	
	//get Apps
	this.getApps = function (data,config) {
        return $http.get($rootScope.gatewayRouter+'/iam/secure/apps/getMyApps',data,config);
